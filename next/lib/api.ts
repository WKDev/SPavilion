 ...
 * @param start - 시작 주소 (기본값: 0)
 * @param count - 조회할 개수 (기본값: 20, 최대: 2000)
...
/**
 * Get all PLC and device statuses in one call
 */
...
export async function getPLCAllStatus(
  coilRange: { start: number; count: number },
  registerRange: { start: number; count: number }
): Promise<AllDeviceStatus> {
  const params = new URLSearchParams();
  params.append('coilStart', coilRange.start.toString());
  params.append('coilCount', coilRange.count.toString());
  params.append('registerStart', registerRange.start.toString());
  params.append('registerCount', registerRange.count.toString());
  
  const response = await fetchApi<AllDeviceStatus>(`/plc/all-status?${params.toString()}`);
  return response;
}

/**
 * Update the background polling ranges on the server
 */
export async function updatePollingRanges(ranges: {
  coilStart?: number;
  coilCount?: number;
  registerStart?: number;
  registerCount?: number;
}): Promise<{ success: boolean }> {
  const response = await fetchApi<{ success: boolean }>('/plc/polling-ranges', {
    method: 'POST',

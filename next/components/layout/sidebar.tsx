    ...
    <Sidebar>
      ...
      <SidebarFooter className="border-t p-4">
        <div className="flex items-center gap-3 rounded-lg bg-muted px-3 py-2">
          <div className={`h-8 w-8 rounded-full ${isPolling ? "bg-primary animate-pulse" : "bg-muted-foreground"}`} />
          <div className="flex-1">
            <p className="text-sm font-medium">Polling Status</p>
            <p className="text-xs text-muted-foreground">{isPolling ? "Active" : "Inactive"}</p>
          </div>
        </div>
      </SidebarFooter>
    </Sidebar>

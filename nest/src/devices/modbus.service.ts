...
export class ModbusService implements OnModuleInit, OnModuleDestroy {
  ...
  private async connectClient(): Promise<void> {
    const port = this.configService.get<string>('modbus.port','COM3');
    const baudRate = this.configService.get<number>('modbus.baudRate', 115200);
    ...
    if (tcpHost) {
      ...
      await this.client.connectRTUBuffered(port, { baudRate: baudRate });

...
export class PlcController {
  ...
  @Get('all-status')
  @ApiOperation({ summary: 'Get all PLC and device statuses in one call' })
  @ApiQuery({ name: 'coilStart', required: false, type: 'number', description: 'Default: 0' })
  @ApiQuery({ name: 'coilCount', required: false, type: 'number', description: 'Default: 20' })
  @ApiQuery({ name: 'registerStart', required: false, type: 'number', description: 'Default: 0' })
  @ApiQuery({ name: 'registerCount', required: false, type: 'number', description: 'Default: 20' })
  @ApiResponse({ status: 200, description: 'All status fetched successfully' })
  async getAllStatus(
    @Query('coilStart') coilStart?: string,
    @Query('coilCount') coilCount?: string,
    @Query('registerStart') registerStart?: string,
    @Query('registerCount') registerCount?: string,
  ) {
    try {
      const coilStartNum = coilStart ? parseInt(coilStart, 10) : undefined;
      const coilCountNum = coilCount ? parseInt(coilCount, 10) : undefined;
      const registerStartNum = registerStart ? parseInt(registerStart, 10) : undefined;
      const registerCountNum = registerCount ? parseInt(registerCount, 10) : undefined;

      let coilRange: { start: number; count: number } | undefined;
      if (coilStartNum !== undefined && coilCountNum !== undefined) {
        coilRange = { start: coilStartNum, count: coilCountNum };
      }

      let registerRange: { start: number; count: number } | undefined;
      if (registerStartNum !== undefined && registerCountNum !== undefined) {
        registerRange = { start: registerStartNum, count: registerCountNum };
      }

      return await this.modbusService.getAllStatus(coilRange, registerRange);
    } catch (error) {
      throw new HttpException(`Failed to get all status: ${error.message}`, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

  @Post('polling-ranges')
  @ApiOperation({ summary: 'Update polling ranges for coils and registers' })
  @ApiBody({
    schema: {
      type: 'object',
      properties: {
        coilStart: { type: 'number' },
        coilCount: { type: 'number' },
        registerStart: { type: 'number' },
        registerCount: { type: 'number' },
      },
    },
  })
  @ApiResponse({ status: 200, description: 'Polling ranges updated successfully' })
  updatePollingRanges(
    @Body()
    body: {
      coilStart?: number;
    ...
    this.modbusService.updatePollingRanges(body);
    return { success: true };
  }
  ...
  async connectPlc(
    ...
    try {
      // Check if already connected with the same settings
      if (this.modbusService.isPlcConnected()) {
        const currentSettings = await this.modbusService.getCurrentConnectionSettings();
        const isSameConnection =
          currentSettings.protocol === body.protocol &&
          (body.protocol === 'modbusTCP'
            ? currentSettings.host === body.host && currentSettings.port === body.port
            : currentSettings.device === body.device && currentSettings.baudRate === body.baudRate);
